...
public:
  ...
  void untagModels(vtkMRMLScene* scene, vtkIdType hierarchyId);
  ...
  void removeTransforms(vtkMRMLScene* scene, vtkIdType hierarchyID);

  void createPlanesIfNecessary(
    vtkMRMLScene* scene, vtkIdType hierarchyID);
  void updatePlanesFromModel(vtkMRMLScene* scene, vtkMRMLModelNode*) const;
  void removePlanes(vtkMRMLScene* scene, vtkIdType hierarchyID);
...
::removeTransforms(vtkMRMLScene* scene, vtkIdType hierarchyID)
{
  vtkMRMLSubjectHierarchyNode* shNode = vtkMRMLSubjectHierarchyNode::GetSubjectHierarchyNode(scene);
  vtkMRMLNode* hierarchyNode = shNode->GetItemDataNode(hierarchyID);

  this->removeTransformNode(scene, hierarchyNode);
...
//-----------------------------------------------------------------------------
void qSlicerPlannerModuleWidgetPrivate
::untagModels(vtkMRMLScene* scene, vtkIdType hierarchyID)
{
  vtkMRMLSubjectHierarchyNode* shNode = vtkMRMLSubjectHierarchyNode::GetSubjectHierarchyNode(scene);
...
void qSlicerPlannerModuleWidgetPrivate::previewCut(vtkMRMLScene* scene)
{
  ...
  //add to hierarchy
  vtkMRMLSubjectHierarchyNode* shNode = vtkMRMLSubjectHierarchyNode::GetSubjectHierarchyNode(scene);
  shNode->CreateItem(this->HierarchyItem, splitNode1);
  shNode->CreateItem(this->HierarchyItem, splitNode2);
...
{
  vtkMRMLSubjectHierarchyNode* shNode = vtkMRMLSubjectHierarchyNode::GetSubjectHierarchyNode(scene);
  vtkIdType itemID = shNode->GetItemByDataNode(node);
  vtkIdType parentItemID = shNode->GetItemParent(itemID);

  if(vtkMRMLSubjectHierarchyNode::INVALID_ITEM_ID != parentItemID)
  {
    shNode->RemoveItem(parentItemID, true, true);
  }
  if(node)
  {
...
void qSlicerPlannerModuleWidget::updateWidgetFromMRML()
{
  ...
  d->createTransformsIfNecessary(this->mrmlScene(), d->HierarchyItem);
  // Create the plane node for the current hierarchy node
  d->createPlanesIfNecessary(this->mrmlScene(), d->HierarchyItem);
  //keeps hierarchy models out of other drop down boxes
  d->tagModels(this->mrmlScene(), d->HierarchyItem);
...
void qSlicerPlannerModuleWidget::onSetPreOP()
{
  ...
  if(vtkMRMLSubjectHierarchyNode::INVALID_ITEM_ID != d->HierarchyItem)
  {
    ...
    if(shNode->GetNumberOfItemChildren(d->HierarchyItem) > 0)
    {
      ...
      d->ActionInProgress[0] = shNode->GetItemName(d->HierarchyItem);
      ...
      d->cmdNode = this->plannerLogic()->createPreOPModels(d->HierarchyItem);
      qvtkReconnect(d->cmdNode, vtkMRMLCommandLineModuleNode::StatusModifiedEvent, this, SLOT(finishWrap()));
...
void qSlicerPlannerModuleWidget::finishDistance()
{
  ...
  std::vector<vtkIdType>::const_iterator it;
  vtkMRMLSubjectHierarchyNode* shNode = vtkMRMLSubjectHierarchyNode::GetSubjectHierarchyNode(this->mrmlScene());
  shNode->GetItemChildren(d->HierarchyItem, children, true);
  for (it = children.begin(); it != children.end(); ++it)
  {
    vtkMRMLModelNode* childModel =
      vtkMRMLModelNode::SafeDownCast(shNode->GetItemDataNode(*it));
...
void qSlicerPlannerModuleWidget::saveDirectoryChanged(const QString &directory)
{
  ...
  if (d->savingActive)
  {
    ...
    vtkMRMLSubjectHierarchyNode* shNode = vtkMRMLSubjectHierarchyNode::GetSubjectHierarchyNode(this->mrmlScene());
    ssDirectoryName << shNode->GetItemName(d->HierarchyItem) << "_" << timestamp.toStdString();
